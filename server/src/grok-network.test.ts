...
test('Grok fixture over backend RPC: disconnect, durable completion, immediate next turn, and approval', async () => {
  const home = mkdtempSync(join(tmpdir(), 'jetty-grok-rpc-'))
  ...
  try {
    ...
    const active = await client.request('turn.start', { threadId, text: 'steer' })
    await subscription.waitFor(
      (message) =>
        message.type === 'event' &&
        message.event.type === 'item.started' &&
