...
describe('translate()', () => {
  ...
  test('streaming send_images tool_use is hidden; sibling tools still emit', () => {
    const ctx = createTranslateCtx('t1')
    translate(
      {
        type: 'stream_event',
    ...
    translate(
      {
        ...
        event: {
          type: 'content_block_delta',
          index: 0,
          delta: { type: 'input_json_delta', partial_json: '{"paths":["a.png"]}' },
        },
      },
      ctx
    )
          ...
          content: [
            {
              type: 'tool_result',
              tool_use_id: 'tu_send',
              content: 'Sent 1 image to the chat: a.png',
              is_error: false,
            },
            {
              type: 'tool_result',
  ...
  test('assistant send_images tool_use is hidden; sibling tools still emit', () => {
    const ctx = createTranslateCtx('t1')
    const started = translate(
      {
        type: 'assistant',
