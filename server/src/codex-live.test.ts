...
test.skipIf(process.env.JETTY_CODEX_LIVE_TEST !== '1')(
  'live Codex backend RPC: tool use, streamed reply, restart and conversation resume',
  async () => {
    const home = mkdtempSync(join(tmpdir(), 'jetty-codex-live-'))
    const marker = `JETTY-${newId()}`
    writeFileSync(join(home, 'marker.txt'), marker)
    let server = await startServer({ home, port: 0, agent: 'codex', titler: null })
    ...
    try {
      ...
      const resume = await Effect.runPromise(server.store.getProviderSessionId(threadId, 'codex'))
      expect(resume).toBeTruthy()
      await client.close()
      await server.stop()
      server = await startServer({ home, port: 0, agent: 'codex', titler: null })
      ...
      expect(await Effect.runPromise(server.store.getProviderSessionId(threadId, 'codex'))).toBe(
        resume
      )
      console.log(
        'Live Codex verified: RPC, streaming, shell tool, persisted resume after backend restart.'
      )
    } finally {
      await client.close()
      await server.stop()
