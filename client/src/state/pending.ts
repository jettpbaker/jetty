...
export type PendingSend = { text: string; projectId: string; phase: 'sending' | 'failed' }
...
export async function sendFirstTurn({
  ...
  pendingSends.set(threadId, { text, projectId, phase: 'sending' })
  try {
    await socket.request('thread.create', { id: threadId, projectId })
    timelineStore.openThread(threadId)
    await socket.request('turn.start', { threadId, text })
    pendingSends.clear(threadId)
    clearDraft(projectId)
  } catch (error) {
    pendingSends.set(threadId, { text, projectId, phase: 'failed' })
